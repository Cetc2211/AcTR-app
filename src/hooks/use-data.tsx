        ...
        try {
            ...
            const uploadToCloud = async <T,>(key: string, data: T, stepName: string): Promise<{ success: boolean; count: number; size: string; error?: string }> => {
                const stepNumber = key === 'app_groups' ? 1 :
                                   key === 'app_students' ? 2 :
                                   key === 'app_observations' ? 3 :
                                   key === 'app_specialNotes' ? 4 :
                                   key === 'app_partialsData' ? 5 : 6;

                setSyncProgress(prev => prev ? {
                    ...prev,
                    step: 'uploading',
                    currentStep: stepNumber,
                    currentTask: `Subiendo ${stepName}...`
                } : null);

                try {
                    const size = getDataSize(data);
                    console.log(`📤 Subiendo ${key}... (${size})`);
                    ...
                    if (bytes > 900000) {
                        console.warn(`⚠️ ${key} es muy grande (${size}), puede tardar más...`);
                        setSyncProgress(prev => prev ? {
                            ...prev,
                            currentTask: `Subiendo ${stepName} (archivo grande: ${size})...`
                        } : null);
                    }
                    ...
                    console.log(`✅ Subido ${key} - ${count} items (${size})`);
                    return { success: true, count, size };
                } catch (error: any) {
                    console.error(`❌ Error subiendo ${key}:`, error);
                    const errorMsg = error?.message || 'Error desconocido';

                    // If it's a size error, try to strip photos
                    if (error.code === 'invalid-argument' || error.message?.includes('too large')) {
                        console.log(`⚠️ Intentando subir ${key} sin fotos...`);
                        setSyncProgress(prev => prev ? {
                            ...prev,
                            currentTask: `Subiendo ${stepName} (sin fotos por tamaño)...`
                        } : null);

                        try {
                            if (key === 'app_students' && Array.isArray(data)) {
                                const strippedData = data.map(s => ({...s, photo: s.photo?.startsWith('data:') ? 'https://placehold.co/100x100.png' : s.photo}));
                                const strippedDocRef = doc(db, 'users', user.uid, 'userData', key);
                                await setDoc(strippedDocRef, { value: strippedData, lastUpdated: now }, { merge: true });
                                console.log(`✅ Subido ${key} sin fotos`);
                                return { success: true, count: strippedData.length, size: 'reduced' };
                            }
                    ...
                    return { success: false, count: 0, size: 'error', error: errorMsg };
                }
            };

            // Upload each collection SEQUENTIALLY with progress
            ...
            console.log(`🎯 Resultado de subida: ${successCount}/${totalCount} exitosos en ${duration}s`);
            ...
            if (successCount === totalCount) {
                ...
                toast({
                    ...
                    description: `${totalItems} items en ${duration}s.`
                });
            } else {
                setSyncStatus('pending');
                const failedItems = results.filter(r => !r.success).map(r => `${r.key}: ${r.error || 'Error'}`).join('; ');
                toast({
                    variant: "destructive",
                    title: "Sincronización parcial",
                    description: `${successCount}/${totalCount} subidos. Errores: ${failedItems}`
                });
            }
